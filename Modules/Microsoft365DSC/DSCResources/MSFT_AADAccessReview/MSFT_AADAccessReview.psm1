function Get-TargetResource
{
    ...
    try
    {
        ...
        if ($null -eq $getValue)
        {
            ...
            {
                $getValue = Get-MgIdentityGovernanceAccessReviewDefinition `
        ...
        {
            $results.Add("Reviewers", (Get-M365DSCDRGComplexTypeToHashtable -ComplexObject $getValue.Reviewers))
        }
        if ($getValue.Scope)
        {
            $results.Add("Scope", (Get-M365DSCDRGComplexTypeToHashtable -ComplexObject $getValue.Scope))
        }
        if ($getValue.Settings)
        {
            $results.Add("Settings", (Get-M365DSCDRGComplexTypeToHashtable -ComplexObject $getValue.Settings))
        }
        if ($getValue.StageSettings)
        {
...
function Export-TargetResource
{
    ...
    try
    {
        ...
        foreach ($config in $getValue)
        {
            ...
            $Results = Update-M365DSCExportAuthenticationResults -ConnectionMode $ConnectionMode `
                -Results $Results
                    ...
                    if ($Results.AdditionalNotificationRecipients)
        {
            $complexTypeStringResult = Get-M365DSCDRGComplexTypeToString -ComplexObject $Results.AdditionalNotificationRecipients -CIMInstanceName MicrosoftGraphaccessreviewnotificationrecipientitem
        ...
        {
            $complexTypeStringResult = Get-M365DSCDRGComplexTypeToString -ComplexObject $Results.Reviewers -CIMInstanceName MicrosoftGraphaccessreviewreviewerscope
            if ($complexTypeStringResult)
            {
        ...
        if ($Results.Scope)
        {
            $complexTypeStringResult = Get-M365DSCDRGComplexTypeToString -ComplexObject $Results.Scope -CIMInstanceName MicrosoftGraphaccessreviewscope
            if ($complexTypeStringResult)
            {
                $Results.Scope = $complexTypeStringResult
            }
            else
            {
                $Results.Remove('Scope') | Out-Null
            }
        }
        if ($Results.Settings)
        {
            $complexTypeStringResult = Get-M365DSCDRGComplexTypeToString -ComplexObject $Results.Settings -CIMInstanceName MicrosoftGraphaccessreviewschedulesettings
            if ($complexTypeStringResult)
            {
        ...
        {
            $complexTypeStringResult = Get-M365DSCDRGComplexTypeToString -ComplexObject $Results.StageSettings -CIMInstanceName MicrosoftGraphaccessreviewstagesettings
            if ($complexTypeStringResult)
            {
        ...
        {
            $isCIMArray=$false
            if($Results.AdditionalNotificationRecipients.getType().Fullname -like "*[[\]]")
            {
                $isCIMArray=$true
            }
            $currentDSCBlock = Convert-DSCStringParamToVariable -DSCBlock $currentDSCBlock -ParameterName "AdditionalNotificationRecipients" -isCIMArray:$isCIMArray
        }
        if ($Results.CreatedBy)
        {
            $isCIMArray=$false
            if($Results.CreatedBy.getType().Fullname -like "*[[\]]")
            {
                $isCIMArray=$true
            }
            $currentDSCBlock = Convert-DSCStringParamToVariable -DSCBlock $currentDSCBlock -ParameterName "CreatedBy" -isCIMArray:$isCIMArray
        }
        if ($Results.FallbackReviewers)
        {
            $isCIMArray=$false
            if($Results.FallbackReviewers.getType().Fullname -like "*[[\]]")
            {
                $isCIMArray=$true
            }
            $currentDSCBlock = Convert-DSCStringParamToVariable -DSCBlock $currentDSCBlock -ParameterName "FallbackReviewers" -isCIMArray:$isCIMArray
        }
        if ($Results.InstanceEnumerationScope)
        {
            $isCIMArray=$false
            if($Results.InstanceEnumerationScope.getType().Fullname -like "*[[\]]")
            {
                $isCIMArray=$true
            }
            $currentDSCBlock = Convert-DSCStringParamToVariable -DSCBlock $currentDSCBlock -ParameterName "InstanceEnumerationScope" -isCIMArray:$isCIMArray
        }
        if ($Results.Reviewers)
        {
            $isCIMArray=$false
            if($Results.Reviewers.getType().Fullname -like "*[[\]]")
            {
                $isCIMArray=$true
            }
            $currentDSCBlock = Convert-DSCStringParamToVariable -DSCBlock $currentDSCBlock -ParameterName "Reviewers" -isCIMArray:$isCIMArray
        }
        if ($Results.Scope)
        {
            $isCIMArray=$false
            if($Results.Scope.getType().Fullname -like "*[[\]]")
            {
                $isCIMArray=$true
            }
            $currentDSCBlock = Convert-DSCStringParamToVariable -DSCBlock $currentDSCBlock -ParameterName "Scope" -isCIMArray:$isCIMArray
        }
        if ($Results.Settings)
        {
            $isCIMArray=$false
        ...
        {
            $isCIMArray=$false
            if($Results.StageSettings.getType().Fullname -like "*[[\]]")
            {
                $isCIMArray=$true
            }
            $currentDSCBlock = Convert-DSCStringParamToVariable -DSCBlock $currentDSCBlock -ParameterName "StageSettings" -isCIMArray:$isCIMArray
        }
        if ($Results.Instances)
        {
            $isCIMArray=$false
            if($Results.Instances.getType().Fullname -like "*[[\]]")
            {
                $isCIMArray=$true
            }
